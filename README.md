...
面向计算机学科研究生的全能学术搜索与评估 Skill，7种模式 + 3项知识增强 + 阅读清单持久化。v3.2

## News

- `2026-05-27` **fix: 链接强制 + 论文级别必标**：设计原则升级为铁律"给不出链接的论文不要列"，所有模板加 CCF 等级+中科院分区列，新增输出自检
- `2026-05-27` **feat: 模式 I — 资源驱动研究规划**：有什么数据/GPU → 能做什么方向 → 论文+代码推荐，自动评估可行性和投稿建议
- `2026-05-27` **feat: 阅读清单持久化**：推荐论文后可一键保存到本地 `scholar-reading-list-[方向].md`，后续对话自动追踪阅读进度
- `2026-05-27` **feat: 搜索前追问**：模式 D/I 开始前先了解用户研几/技术栈/GPU/目标，精准推荐
- `2026-05-27` **feat: 四级发散搜索**：模式 D 不再窄化——同领域直搜 → 同类问题×其他文字 → 通用方法 → 跨域启发，避免"用户说满文只推荐满文"的问题
- `2026-05-27` **feat: 所有推荐附链接**：模式 D/E/F 的论文/代码/数据集必须附 arXiv/DOI/GitHub 链接，可点击验证
- `2026-05-27` **feat: 多平台适配**：新增 `references/platform-adapter.md`，支持 Claude Code / Codex / Cursor / 纯API 四种环境
- `2026-05-27` **feat: 参考文件**：新增 `references/ccf-rankings.md`（CCF第七版分级速查）和 `references/search-patterns.md`（搜索语法速查）
- `2026-05-26` **feat: v3.0 初始发布**：6种模式（C-H）+ 3项知识增强 + 降级策略 + GitHub搜索语法 + 11条陷阱速查
